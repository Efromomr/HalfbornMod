...
{
    public class GreenElemental : Elemental
    {
        public override void SetStaticDefaults()
        {
            this.DisplayName.SetDefault("Green Elemental");
        ...
        public override void SetDefaults()
        {
            ...
            banner = npc.type;
            bannerItem = mod.ItemType("GreenElementalBanner");
        }
        public override void HitEffect(int hitDirection, double damage)
        {
            if (this.npc.life <= 0)
            {
                ...
                for (int index = 0; index < 5; ++index)
                    Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/GreenElemGore"), 1f);
            }
            else
            {
        ...
        public override void NPCLoot()
        {
            if (Main.rand.Next(15) == 0)
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("GreenShard"), Main.rand.Next(1, 2), false, 0, false, false);
        }
    }
}