...
{
    public class BlueElemental : Elemental
    {
        public override void SetStaticDefaults()
        {
            this.DisplayName.SetDefault("Blue Elemental");
        ...
        public override void SetDefaults()
        {
            ...
            banner = npc.type;
            bannerItem = mod.ItemType("BlueElementalBanner");
        }
        public override void HitEffect(int hitDirection, double damage)
        {
            if (this.npc.life <= 0)
            {
                ...
                for (int index = 0; index < 5; ++index)
                    Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/BlueElemGore"), 1f);
            }
            else
            {
        ...
        public override void NPCLoot()
        {
            if (Main.rand.Next(15) == 0)
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("BlueShard"), Main.rand.Next(1, 2), false, 0, false, false);
        }
    }
}